...
function waDefaultMsg(o) {
  const products = (o.sets || []).map(s =>
    `סט טקטי חולצה ${s.shirtSize} + מכנסיים ${s.pantsSize}${(s.quantity||1)>1 ? ` ×${s.quantity}` : ''}`
  ).join(', ') || 'סט טקטי';
  return `שלום ${o.name}, בהמשך להזמנתך באתר למדים טקטיים, הזמנה #${o.orderNumber || ''} על סך ₪${o.total || 0} עבור ${products} לא הושלמה.\nבמידה ואתה רוצה לבצע את ההזמנה אנא העבר הסכום באחד מאמצעי התשלום הבאים: PAYBOX, BIT, או העברה בנקאית`;
}
